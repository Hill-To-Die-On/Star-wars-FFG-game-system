...
Relationships use stable node IDs and an allow-listed directed kind. Alongside character and organisation ties, the contract includes vehicle and home-base relationships: `owns`, `pilots`, `crews`, `aboard`, `docked-at` and `based-at`. The optional creator label remains separate from the kind. Runtime validation rejects dangling references, reused Actor references, self-links, duplicate IDs and unknown fields before any Actor is created.
