...
| Optional Foundry interface theme             | Passed for Frontier, Rebellion and Mystic across the cinematic backdrop, six custom navigation icons, pause mark, chat, populated combat tracker, windows, journals/handouts, scene controls, players and hotbar; automatic selection, manual overrides, Foundry-default opt-out, local asset loading and zero page errors verified |
| Chat dice shapes and theme contrast          | Automated checks pass all three themes at WCAG 2.1 AA text contrast and verify seven semantic dice classes with square, diamond, twelve-sided and circular silhouettes; refreshed Foundry visual acceptance recorded with the current build |
