...
| SW Adversaries file import | Passed through the Foundry menu: 34 native actors, repeat import preserved all 34, missing weapon statistics remained references, DoR received actor and ability facts |
| Bundled interface fonts | Roboto and Signika loaded from local assets; computed sheet fonts and rendered headings verified in Chromium |
...
The initial Node suite has 18 passing tests. `tests/foundry-smoke.mjs` is an opt-in real-browser test requiring an authenticated, isolated `starfall-validation` world. Set `FOUNDRY_URL`, `FOUNDRY_STORAGE_STATE`, and optionally `CHROMIUM_PATH`; it creates one synthetic actor. Never point it at a campaign world.
