...
The native provider supplies actor facts, book configuration, structured motivations, legal talent and signature-ability purchases, signature link/unlock state, learned guidance, per-rule automation status, check guidance, token-to-token range and explicit skill-pool execution. `getCombatRange(sourceToken, targetToken)` returns the Personal, Battlefield/planetary or Ship/vehicle/space band from the same scene profile used by the visible overlay and character-sheet pool builder. `getCheckTalentRules(actor, skill)` returns applied passive modifiers, fixed result symbols and active decisions; `executeCheck` uses the same `actor.rollSkill` path as the character sheet. Structured passive effects therefore cannot diverge between DoR and manual play. Motivations guide portrayal, hooks and source-defined rewards but do not invent dice modifiers. Guidance-only abilities remain visible to DoR and require explicit adjudication.

The private structured dataset currently gives all 2,700 imported specialization nodes player guidance, 36 of 38 signature-ability graphs, private guidance for 298 motivation references and automatic dice/result effects for 28 unique passive talents. Six signature graphs still contain at least one node with only a book reference; Peerless Interception and Unmatched Teamwork lack source graphs. Other active talent and signature effects, weapon qualities, Force-power upgrades, critical consequences, elevation/line of sight and vehicle maneuvers still need additional codification or source adjudication. The adapter can now obtain a native range band, but `executeAttack` still requires an explicit weapon, target defence and GM-approved final pool. Full unattended play has not passed acceptance.
